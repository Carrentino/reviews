...
@common_router.post('/review/reply/', status_code=status.HTTP_201_CREATED)
async def create_review_reply(
    ...
    if req.type == ReviewType.CAR:
        try:
            return await car_review_service.create_review_reply(UUID(user_context.user_id), req)
    ...
    if req.type == ReviewType.USER:
        try:
            return await user_review_service.create_review_reply(UUID(user_context.user_id), req)
        except ReviewNotFoundError:
            raise ReviewNotFoundHttpError from None
        except UserIsNotOwnerError:
            raise UserIsNotOwnerHttpError from None
